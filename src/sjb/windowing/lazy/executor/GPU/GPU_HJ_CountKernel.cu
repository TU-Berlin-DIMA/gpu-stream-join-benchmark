...
namespace Windowing {
    namespace Lazy {
        namespace Executor {
            GPU_HJ_CountKernel::GPU_HJ_CountKernel(
            ...
            ExecutionStatistic GPU_HJ_CountKernel::execute(Tuple *leftRingBuffer, Tuple *rightRingBuffer,
                ...
                launchBuildHashTableKernel(d_leftTuples, nLeftTuples, d_hashTable,
                                           h_prefixSum, d_occupation, numDistinctKeys, maxTupleInWindow,
                                           joinerStream);
                CUDA_CHECK(cudaStreamSynchronize(joinerStream));
                ...
                launchProbeCountKernel(d_rightTuples, nRightTuples, nLeftTuples,
                                       d_hashTable, d_histogram, h_prefixSum,
                ...
                GPU_HJ::launchProbeJoinKernel(d_rightTuples, nRightTuples, nLeftTuples, d_hashTable, d_histogram,
                                              h_prefixSum, d_resultCount, numDistinctKeys, d_resultTuple, joinerStream);
                CUDA_CHECK(cudaStreamSynchronize(joinerStream));
            ...
            GPU_HJ_CountKernel::~GPU_HJ_CountKernel() = default;
        }
    }
}