...
namespace Windowing {
    namespace Lazy {
        namespace Executor {
            GPU_SMJ_Atomic::GPU_SMJ_Atomic(uint64_t numDistinctKeys, uint64_t batchSize,
                                           uint64_t maxTupleInWindow) :
                    GPU_SMJ(numDistinctKeys, batchSize, maxTupleInWindow) {

            }

            GPU_SMJ_Atomic::~GPU_SMJ_Atomic() = default;

            ExecutionStatistic
            GPU_SMJ_Atomic::execute(Tuple *leftRingBuffer, Tuple *rightRingBuffer,
                ...
                launchSortKernel(d_rightTuples, d_sortedRightTuples, d_rightPrefixSum, nRightTuples, d_rightOccupation,
                                 rightStream);
                CUDA_CHECK(cudaStreamSynchronize(rightStream));
                ...
                launchMergeWriteKernel(d_leftHistogram, d_rightHistogram, d_sortedLeftTuples, d_sortedRightTuples,
                                       sink.counterGPU, numDistinctKeys, joinerStream, sink.sinkBuffer, sink.sinkBufferSize);
