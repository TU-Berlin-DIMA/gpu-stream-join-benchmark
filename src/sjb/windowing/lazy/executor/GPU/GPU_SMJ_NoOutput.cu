...
namespace Windowing {
    namespace Lazy {
        namespace Executor {
            GPU_SMJ_NoOutput::GPU_SMJ_NoOutput(uint64_t numDistinctKeys, uint64_t batchSize,
                                               uint64_t maxTupleInWindow) :
                    GPU_SMJ(numDistinctKeys, batchSize, maxTupleInWindow) {

            }

            GPU_SMJ_NoOutput::~GPU_SMJ_NoOutput() = default;

            ExecutionStatistic
            GPU_SMJ_NoOutput::execute(Tuple *leftRingBuffer, Tuple *rightRingBuffer,
